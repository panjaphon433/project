  ...
  .controller('Appoint2Controller', ['$scope','$http','$filter','$location', function($scope,$http,$filter,$location){
       ...
       // $scope.s.date
        $scope.select= function(){{ 
            ...
            // }
            // else{
            //     if(!$scope.s.key) // key null date ข้อมูลตามหน้าจอ
             
                    //$scope.s  = {'key': "", 'date':};
                    $scope.s.date = $filter('date')($scope.s.date0, "yyyy-MM-dd");
                    //$scope.s.date =  $scope.s.date.toISOString().split("T",2)[0];
                    //$scope.s.date = new Date ($scope.s.date);

                    // $scope.s.date = $filter('date')($scope.s.date, "yyyy-MM-dd");
                    // $scope.s.key = "";
              
                // else if(!$scope.s.date){ // date null key ข้อมูลตามหน้าจอ 
                //     //$scope.s  = {'key':"" , 'date':""};
                //     $scope.s.date = "";
                // }
                // else{
